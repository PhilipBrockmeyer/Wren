...
namespace Wren.Emulation.Nes
{
    ...
    public class Cpu6502
    {
        ...
        public bool PauseDone;//Very important for save / load state.
        ...
        {
            return _bus.ReadMemory8(c);
        }
        public byte ZeroPageX(ushort c)
        {
            return _bus.ReadMemory8((ushort)(0xff & (c + x_index_register)));
        }
        public byte ZeroPageY(ushort c)
        {
            return _bus.ReadMemory8((ushort)(0xff & (c + y_index_register)));
        }
        public byte Absolute(byte c, byte d)
        {
            return _bus.ReadMemory8(MakeAddress(c, d));
        }
        public byte AbsoluteX(byte c, byte d, bool check_page)
        {
            ...
            return _bus.ReadMemory8((ushort)(MakeAddress(c, d) + x_index_register));
        }
        public byte AbsoluteY(byte c, byte d, bool check_page)
        {
            ...
            return _bus.ReadMemory8((ushort)(MakeAddress(c, d) + y_index_register));
        }
        public byte IndirectX(byte c)
        {
            return _bus.ReadMemory8((ushort)_bus.ReadMemory16((ushort)(0xff & (c + (ushort)x_index_register))));
        }
        public byte IndirectY(byte c, bool check_page)
        {
            if (check_page)
            {
                if ((_bus.ReadMemory16(c) & 0xFF00) !=
                    ((_bus.ReadMemory16(c) + y_index_register) & 0xFF00))
                {
                    tick_count += 1;
                };
            }
            return _bus.ReadMemory8((ushort)(_bus.ReadMemory16(c) + (ushort)y_index_register));
        }

        public byte ZeroPageWrite(ushort c, byte data)
        {
            return _bus.WriteMemory8(c, data);
        }
        public byte ZeroPageXWrite(ushort c, byte data)
        {
            return _bus.WriteMemory8((ushort)(0xff & (c + x_index_register)), data);
        }
        public byte ZeroPageYWrite(ushort c, byte data)
        {
            return _bus.WriteMemory8((ushort)(0xff & (c + y_index_register)), data);
        }
        public byte AbsoluteWrite(byte c, byte d, byte data)
        {
            return _bus.WriteMemory8(MakeAddress(c, d), data);
        }
        public byte AbsoluteXWrite(byte c, byte d, byte data)
        {
            return _bus.WriteMemory8((ushort)(MakeAddress(c, d) + x_index_register), data);
        }
        public byte AbsoluteYWrite(byte c, byte d, byte data)
        {
            return _bus.WriteMemory8((ushort)(MakeAddress(c, d) + y_index_register), data);
        }
        public byte IndirectXWrite(byte c, byte data)
        {
            return _bus.WriteMemory8((ushort)_bus.ReadMemory16((ushort)(0xff & (c + (short)x_index_register))), data);
        }
        public byte IndirectYWrite(byte c, byte data)
        {
            return _bus.WriteMemory8((ushort)(_bus.ReadMemory16(c) + (ushort)y_index_register), data);
        }
        #endregion
        #region Operations (pull, Push ...)
        public void Push8(byte data)
        {
            _bus.WriteMemory8((ushort)(0x100 + sp_register), (byte)(data & 0xff));
        ...
        {
            sp_register = (byte)(sp_register + 1);
            return _bus.ReadMemory8((ushort)(0x100 + sp_register));
        }
        ...
        private void OpcodeADC()
        {
            // We may not use both, but it's easier to grab them now
            byte arg1 = _bus.ReadMemory8((ushort)(pc_register + 1));
            byte arg2 = _bus.ReadMemory8((ushort)(pc_register + 2));
        ...
        private void OpcodeBNE()
        {
            ...
            if (zero_flag == 0)
            {
                arg1 = _bus.ReadMemory8((ushort)(pc_register + 1));
                pc_register += 2;
                if ((pc_register & 0xFF00) != ((pc_register + (sbyte)arg1 + 2) & 0xFF00))
                {
                    tick_count += 1;
        ...
        private void OpcodeBRK()
        {
            ...
            PushStatus();
            interrupt_flag = 1;
            pc_register = _bus.ReadMemory16((ushort)0xfffe);
            tick_count += 7;
        }
        private void OpcodeBVC()
        {
            byte arg1 = _bus.ReadMemory8((ushort)(pc_register + 1));
        ...
        private void OpcodeCLC()
        {
            carry_flag = 0;
            pc_register += 1;
            tick_count += 2;
        }
        private void OpcodeCLD()
        {
            decimal_flag = 0;
            pc_register += 1;
            tick_count += 2;
        }
        private void OpcodeCLI()
        {
            interrupt_flag = 0;
            pc_register += 1;
            tick_count += 2;
        }
        private void OpcodeCLV()
        {
            overflow_flag = 0;
            pc_register += 1;
            tick_count += 2;
        }
        private void OpcodeCMP()
        {
            // We may not use both, but it's easier to grab them now
            byte arg1 = _bus.ReadMemory8((ushort)(pc_register + 1));
            byte arg2 = _bus.ReadMemory8((ushort)(pc_register + 2));
        ...
        private void OpcodeLDA()
        {
            ...
            switch (currentOpcode)
            {
                ...
                case (0xAD): arg2 = _bus.ReadMemory8((ushort)(pc_register + 2));
                    a_register = Absolute(arg1, arg2);
                    tick_count += 4; pc_register += 3; break;
                case (0xBD): arg2 = _bus.ReadMemory8((ushort)(pc_register + 2));
                    a_register = AbsoluteX(arg1, arg2, true);     //CHECK FOR PAGE BOUNDARIES


                    tick_count += 4; pc_register += 3; break;
                case (0xB9): arg2 = _bus.ReadMemory8((ushort)(pc_register + 2));
        ...
        private void OpcodePHA()
        {
            Push8(a_register);
            pc_register += 1;
            tick_count += 3;
        }
        private void OpcodePHP()
        {
            PushStatus();
            pc_register += 1;
            tick_count += 3;
        }
        private void OpcodePLA()
        {
            a_register = Pull8();
            if ((a_register & 0xff) == 0)
                zero_flag = 1;
        ...
        private void OpcodePLP()
        {
            PullStatus();
            pc_register += 1;
            tick_count += 4;
        }
        private void OpcodeROL()
        {
            // We may not use both, but it's easier to grab them now
            byte arg1 = _bus.ReadMemory8((ushort)(pc_register + 1));
            byte arg2 = _bus.ReadMemory8((ushort)(pc_register + 2));
        ...
        private void OpcodeRTI()
        {
            PullStatus();
            pc_register = Pull16();
            tick_count += 6;
        }
        private void OpcodeRTS()
        {
            pc_register = Pull16();
            tick_count += 6;
            pc_register += 1;
        }
        private void OpcodeSBC()
        {
            // We may not use both, but it's easier to grab them now
            byte arg1 = _bus.ReadMemory8((ushort)(pc_register + 1));
            byte arg2 = _bus.ReadMemory8((ushort)(pc_register + 2));
        ...
        private void OpcodeSEC()
        {
            carry_flag = 1;
            tick_count += 2;
            pc_register += 1;
        }
        private void OpcodeSED()
        {
            decimal_flag = 1;
            tick_count += 2;
            pc_register += 1;
        }
        private void OpcodeSEI()
        {
            interrupt_flag = 1;
            tick_count += 2;
            pc_register += 1;
        }
        private void OpcodeSTA()
        {
            // We may not use both, but it's easier to grab them now
            byte arg1 = _bus.ReadMemory8((ushort)(pc_register + 1));
            byte arg2 = _bus.ReadMemory8((ushort)(pc_register + 2));
        ...
        public UInt32 RunNextInstruction()
        {
            previousPC = pc_register;
            currentOpcode = _bus.ReadMemory8(pc_register);
            tick_count = 0;
            RunInstruction(currentOpcode);
            return tick_count;
        }
